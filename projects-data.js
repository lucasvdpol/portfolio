...
function makeThumbnailSVG(p, w = 640, h = 360) {
...
function projectCard(p) {
  return `
    <a class="project-card" href="project-detail.html?id=${p.id}">
      <div class="project-thumb-svg">${makeThumbnailSVG(p)}</div>
      <div class="project-body">
        <div class="project-tags">${p.tags.map(t => `<span class="tag">${t}</span>`).join('')}</div>
        <h3>${p.title}</h3>
        <p>${p.short}</p>
        <div class="project-links">
          ${p.live     ? `<span class="project-link primary">Live ↗</span>` : ''}
          ${p.playstore ? `<span class="project-link store android">▶ Play Store</span>` : ''}
          ${p.appstore  ? `<span class="project-link store apple">⬡ App Store</span>` : ''}
        </div>
      </div>
    </a>`;
}